...
def process_project(project_name, config, token, issue_completion_key):
    ...
    df_merged.drop(columns=["Issue Key", "Status", "Created", "Resolved"], inplace=True)
    df_merged.to_excel(merged_path, index=False)
    print(
        f"Full dataset for {project_name} containing 13 dev features and (summary & description) saved to: {merged_path}"
    )
